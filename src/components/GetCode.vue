<template>
  <div class="login-page">
    ...
    <el-card class="login-card" shadow="hover">
      ...
      <el-form ref="formRef" label-width="0px" class="login-form">
        <el-form-item>
          <el-input v-model="phone" placeholder="请输入账号或手机号" size="large" prefix-icon="User" clearable/>
        </el-form-item>
      ...
      <div class="link-row">
        <router-link to="/login" class="forget-link">← 返回登录页</router-link>
      </div>
    </el-card>
  </div>
</template>
...
const next = async () => {
  ...
  checking.value = true
  try {
    await checkCode({ phone: phone.value, checkCode: code.value })
    // 验证通过，携带手机号跳转重置密码页
    router.push({
      path: '/forgetPassword',
